    ...
    constructor() {
        ...
        this.groupCreate = async (title, participants) => {
            ...
            try {
                ...
                this.log(`error in group creation: ${error}, switching gid & checking`, Constants_1.MessageLogLevel.info);
                // if metadata is not available
                const comps = gid.replace('@g.us', '').split('-');
                response.gid = `${comps[0]}-${+comps[1] + 1}@g.us`;
                await this.groupMetadata(gid);
                this.log(`group ID switched from ${gid} to ${response.gid}`, Constants_1.MessageLogLevel.info);
            }
